...
const ColorPip = ({ color }) => {
    const colorClasses = {
        Red: 'bg-red-500', Green: 'bg-green-500', Purple: 'bg-purple-500', Blue: 'bg-blue-500', Yellow: 'bg-yellow-500', Black: 'bg-gray-800', White: 'bg-slate-200'
    };
...
function validate(mainDeck, lifeDeck) {
  ...
  if (mainDeck.length !== RULES.main.size) {
    problems.push(`Main Deck ต้องมี ${RULES.main.size} ใบ (ปัจจุบัน ${mainDeck.length})`);
    ok.main = false;
  }
  ...
  if (overCopies.length) {
    overCopies.forEach(([k, n]) => problems.push(`การ์ด “${k}” ใน Main Deck ซ้ำเกิน ${RULES.main.maxCopiesPerName} ใบ (มี ${n} ใบ)`));
    ok.main = false;
  }
  const onlyRank1Count = mainDeck.filter((c) => c.onlyRank === 1).length;
    ...
    problems.push(`Main Deck สามารถมี “Only #1” ได้แค่ ${RULES.main.requireOnlyRank1Exactly} ใบ (ปัจจุบัน ${onlyRank1Count})`);
    ok.main = false;
  }
  if (lifeDeck.length !== RULES.life.size) {
    problems.push(`Life Deck ต้องมี ${RULES.life.size} ใบ (ปัจจุบัน ${lifeDeck.length})`);
    ok.life = false;
  }
  if (RULES.life.uniqueNames) {
    ...
    if (duplicateLife.length) {
      duplicateLife.forEach(([k]) => problems.push(`การ์ดใน Life Deck ต้องชื่อไม่ซ้ำกัน: “${k}”`));
      ok.life = false;
    }
  }
  return { problems, ok };
    ...
    <CardShell ref={cardItemRef} className={`flex flex-col card group relative ${isDragging ? 'opacity-0' : ''} transition-transform duration-200 ease-in-out ${hoverClasses}`}>
      ...
      <div className="flex-grow flex flex-col justify-between">
        ...
        <div className="mt-2 pt-2 border-t border-emerald-500/20 space-y-2 opacity-0 max-h-0 group-hover:opacity-100 group-hover:max-h-full transition-all duration-300 ease-in-out">
          ...
          <div className="pt-2">
            <p className="text-base text-gray-400">ฝ่าย: <span className="font-semibold text-gray-200">{card.faction ?? 'ไม่มี'}</span></p>
            <p className="text-base text-gray-300 font-light mt-1">{card.text || 'ไม่มีเอฟเฟ็ค'}</p>
            {card.flavor && (
                <p className="text-sm text-amber-200/70 italic mt-2 font-light">"{card.flavor}"</p>
            )}
          </div>
        </div>
      </div>
      ...
      <div ref={dropRef}>
        <div className="flex items-center justify-between mb-2 gap-2">
            ...
            {onViewDeck && deck.length > 0 && (
                <button onClick={onViewDeck} className="text-xs text-amber-400 hover:text-amber-200 hover:underline transition">ดูเด็ค</button>
            )}
        ...
        <div className="relative w-full transition-all" style={{ height: `${containerHeight}px` }}>
          {groupedDeck.length === 0 ? (
            <div className="absolute inset-0 flex items-center justify-center text-slate-500 text-xs">ลากการ์ดมาวางที่นี่</div>
          ) : (
              ...
              {groupedDeck.map(({ card, count }, index) => {
                ...
                return (
                  <div key={`${card.id}-${index}`} className="absolute transition-all duration-200 ease-in-out group hover:-translate-y-2 hover:z-50"
                    ...
                    title={`คลิกเพื่อลบ ${card.name}`} onClick={() => onRemoveCard(card)} >
        ...
        <div className="fixed inset-0 bg-black/80 backdrop-blur-md flex items-center justify-center z-[200] p-4">
          <div className="bg-slate-900/70 border border-emerald-500/30 rounded-xl shadow-2xl w-full h-full flex flex-col">
            ...
            <div className="flex-grow overflow-y-auto p-4">
                {groupedDeck.length === 0 ? (
                    <div className="flex items-center justify-center h-full"><p className="text-slate-400">เด็คนี้ว่างเปล่า</p></div>
                ) : (
    ...
    return createPortal(
        <div className="fixed inset-0 bg-black/80 backdrop-blur-md flex items-center justify-center z-[250] p-4">
            <div className="bg-slate-900/80 border border-emerald-500/30 rounded-xl shadow-2xl w-full h-full flex flex-col max-w-6xl max-h-[90vh]">
                <header className="flex items-center justify-between p-4 border-b border-emerald-500/20 shrink-0">
                    <h2 className="text-2xl font-bold text-white">ผลลัพธ์การสร้างเด็ค</h2>
                    <Button onClick={onClose}>Close</Button>
                </header>
                ...
                <div className="flex-grow overflow-hidden grid grid-cols-1 md:grid-cols-3 gap-6 p-6">
                    <div className="md:col-span-1 flex flex-col gap-6 overflow-y-auto pr-2">
                        ...
                        <div>
                            <h3 className="text-xl font-semibold text-amber-300 border-b border-amber-400/20 pb-1 mb-3">รหัส Export</h3>
                     ...
                     <div className="md:col-span-2 overflow-y-auto pr-2 border-l border-emerald-500/20 pl-6">
                         <h3 className="text-xl font-semibold text-amber-300 border-b border-amber-400/20 pb-1 mb-4">การ์ดในเด็ค ({mainDeck.length} ใบ)</h3>
...
function LeftSidebar({
  ...
  allCardTypes, filterTypes, setFilterTypes,
  filterMagicType, setFilterMagicType,
  allColorTypes, filterColors, setFilterColors,
  allRarities, filterRarities, setFilterRarities,
    ...
    <aside className="w-full h-full flex flex-col p-4 bg-black/40 md:w-[360px] md:shrink-0 md:border-r border-emerald-700/30 backdrop-blur-lg z-30">
      <div className="flex-1 overflow-y-auto pr-2 space-y-4">
        <div>
            ...
            <input type="search" placeholder="ค้นหาชื่อการ์ดหรือข้อความ..." className="w-full px-4 py-2 border border-emerald-500/30 rounded-lg focus:ring-2 focus:ring-amber-500 focus:outline-none transition bg-slate-700/50 placeholder-gray-400 text-white" value={searchTerm} onChange={(e) => setSearchTerm(e.target.value)} />
        </div>
        
        {allCardTypes.length > 0 && (
          <div className="flex flex-wrap gap-2">
            <button onClick={() => setFilterTypes([])} className={`px-3 py-1 text-sm rounded-full transition-colors ${filterTypes.length === 0 ? 'bg-amber-500 text-white font-semibold shadow' : 'bg-slate-700 hover:bg-slate-600 text-gray-300'}`}>All</button>
            {allCardTypes.map((type) => (<button key={type} onClick={() => handleToggle(setFilterTypes, type)} className={`px-3 py-1 text-sm rounded-full transition-colors ${filterTypes.includes(type) ? 'bg-amber-500 text-white font-semibold shadow' : 'bg-slate-700 hover:bg-slate-600 text-gray-300'}`}>{type}</button>))}
          </div>
        )}
        
        {filterTypes.includes('Magic') && (
            <div className="pl-4 mt-2 border-l-2 border-slate-600">
                 <h3 className="text-sm font-semibold text-gray-400 mt-2 mb-2 uppercase tracking-wider">Magic Type</h3>
                <div className="flex flex-wrap gap-2">
        ...
        <div>
            ...
            <div className="flex flex-wrap gap-2">
                <button onClick={() => setFilterColors([])} className={`px-3 py-1 text-sm rounded-full transition-colors ${filterColors.length === 0 ? 'bg-amber-500 text-white font-semibold shadow' : 'bg-slate-700 hover:bg-slate-600 text-gray-300'}`}>All</button>
                {allColorTypes.map((color) => (
                    <button key={color} onClick={() => handleToggle(setFilterColors, color)} className={`px-3 py-1 text-sm rounded-full transition-colors ${filterColors.includes(color) ? 'bg-amber-500 text-white font-semibold shadow' : 'bg-slate-700 hover:bg-slate-600 text-gray-300'}`}>{color}</button>
                ))}
            </div>
        </div>
        )}
        ...
        <div>
            ...
            <div className="flex flex-wrap gap-2">
                <button onClick={() => setFilterRarities([])} className={`px-3 py-1 text-sm rounded-full transition-colors ${filterRarities.length === 0 ? 'bg-amber-500 text-white font-semibold shadow' : 'bg-slate-700 hover:bg-slate-600 text-gray-300'}`}>All</button>
                {allRarities.map((rarity) => (
                    <button key={rarity} onClick={() => handleToggle(setFilterRarities, rarity)} className={`px-3 py-1 text-sm rounded-full transition-colors ${filterRarities.includes(rarity) ? 'bg-amber-500 text-white font-semibold shadow' : 'bg-slate-700 hover:bg-slate-600 text-gray-300'}`}>{rarity}</button>
                ))}
            </div>
        </div>
        )}
...
function CardGrid({ cards, onDoubleClick, onViewDetails, onAddCard }) {
  if (cards.length === 0) {
    return (<CardShell><div className="text-center py-16 text-slate-300">ไม่พบการ์ดตามเงื่อนไข</div></CardShell>);
  }
  return (
...
const CARD_PATHS = ["000.PRE ORDER (PRE0)/PRE0", "001.PROMO CARD (PRM0)/PRM0", "002.STARTER DECK (SD01 - SD07)/SD01 - ตัวตึงไกรลาส", "002.STARTER DECK (SD01 - SD07)/SD02 - วีรบุรุษปากซอย", "002.STARTER DECK (SD01 - SD07)/SD03 - นรกก็แค่น้ำพริก", "002.STARTER DECK (SD01 - SD07)/SD04 - ทหารไก่ชนเขา", "002.STARTER DECK (SD01 - SD07)/SD05 - กำเนิดจากน้ำ", "002.STARTER DECK (SD01 - SD07)/SD06 - ๖ ประจัญบาน", "002.STARTER DECK (SD01 - SD07)/SD07 - VS 18 หัวเมือง", "003.BOOSTER (BT01 - BT07)/BT01 - Welcome ตลิ่งชัน", "003.BOOSTER (BT01 - BT07)/BT02 - Attack on เพื่อนบ้าน", "003.BOOSTER (BT01 - BT07)/BT03 - อมนุษย์ Invasion", "003.BOOSTER (BT01 - BT07)/BT04 - ความจริง Today", "003.BOOSTER (BT01 - BT07)/BT05 - Culture ช๊อค", "003.BOOSTER (BT01 - BT07)/BT06 - โลกา Amagedon", "003.BOOSTER (BT01 - BT07)/BT07 - Life of หน่วง", "004.COMMUNITY COLLECTION (CC01)/CC01 - Community Collection", "005.SELECTION (SL01)", "006.ODENYA (ODY1) - REPRINT/ODY1 - Odenya",];
...
export default function App() {
  ...
  const addToLife = (card) => {
    if (!card.lifeEligible) { showAlert("Invalid Card", `การ์ด "${card.name}" ไม่สามารถใส่ใน Life Deck ได้`); return; };
    if (lifeDeck.length >= RULES.life.size) { showAlert("Deck Full", `Life Deck เต็มแล้ว (ใส่ได้ ${RULES.life.size} ใบ)`); return; };
    if (!lifeDeck.some(c => nameKey(c.name) === nameKey(card.name))) { setLifeDeck(prev => [...prev, card]); } 
    else { showAlert("Duplicate Card", `การ์ดชื่อ "${card.name}" มีใน Life Deck แล้ว (ชื่อห้ามซ้ำ)`); }
  };
  ...
  const handleAnimationComplete = () => {
   ...
   const handleExportCode = () => {
       if (mainDeck.length === 0 && lifeDeck.length === 0) { showAlert("Empty Deck", "เด็คของคุณว่างเปล่า ไม่มีอะไรให้ Export"); return; }
       const code = encodeDeckCode(mainDeck, lifeDeck);
       navigator.clipboard.writeText(code).then(() => showAlert("Success!", `คัดลอกรหัสเด็ค (${code}) ลง Clipboard แล้ว!`));
   };

    const handleImportCode = () => {
        const code = prompt("วางรหัสเด็คที่นี่:");
        if (!code) return;
        ...
        if (decoded) {
            ...
            showAlert("Import Error", "รหัสเด็คไม่ถูกต้อง หรือไม่พบการ์ดบางใบ");
        }
    };

    const handleExport = handleExportCode;
    const handleImport = handleImportCode;

  const handleClear = () => {
    setModal({ isOpen: true, title: "Confirm Clear Deck", message: "คุณต้องการล้างเด็คทั้งหมด (Main และ Life) ใช่หรือไม่? การกระทำนี้ไม่สามารถย้อนกลับได้", onConfirm: () => { setMainDeck([]); setLifeDeck([]); closeModal(); } });
  };
          ...
          <main className="flex-1 flex flex-col md:flex-row overflow-hidden">
            ...
            <div className={`${activeView === 'cards' ? 'flex' : 'hidden'} md:flex flex-1 flex-col overflow-hidden`}>
                <section className="flex-1 p-4 lg:p-6 overflow-y-auto">
                    ...
                    {cardDb.length === 0 ? (<CardShell><div className="text-center py-20"><h3 className="text-2xl font-bold text-white mb-2">กำลังโหลดฐานข้อมูลการ์ด...</h3><p className="text-gray-300">หากไม่สำเร็จ ลองกด "Reload from TXT" ที่แถบซ้าย</p></div></CardShell>) : (<CardGrid cards={filteredCardDb} onDoubleClick={handleCardDoubleClick} onViewDetails={setZoomedCard} onAddCard={addToMain} />)}
                </section>
            </div>
          </main>
