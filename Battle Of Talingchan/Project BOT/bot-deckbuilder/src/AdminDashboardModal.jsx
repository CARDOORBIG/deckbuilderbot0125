...
export default function AdminDashboardModal({ isOpen, onClose, adminEmail }) {
  const [activeTab, setActiveTab] = useState('system'); // เริ่มต้นที่หน้า System
  ...
  const updateTopupStatus = async (status) => {
      // ใช้ upsert เพื่อความชัวร์ (ถ้าไม่มีแถวนี้จะสร้างใหม่ให้เลย)
      const { error } = await supabase
          .from('system_config')
          .upsert({ key: 'topup_status', value: status });
      ...
      if (error) {
          ...
          // alert("✅ อัปเดตสถานะเรียบร้อย!"); // ไม่ต้อง alert ก็ได้ถ้า UI เปลี่ยนสีแล้ว
      }
  };
  ...
  const fetchAllUsers = async () => {
    ...
    } catch (error) { console.error(error); alert("โหลดข้อมูล Users ไม่สำเร็จ"); } finally { setIsLoadingData(false); }
  };
  ...
  const fetchFeedbacks = async () => {
    ...
    } catch (error) { console.error(error); alert("โหลดข้อมูล Feedback ไม่สำเร็จ"); } finally { setIsLoadingData(false); }
  };

  useEffect(() => {
    if (isOpen) {
        fetchSystemConfig(); // โหลดค่า Config ทุกครั้งที่เปิด
      ...
      <div className="bg-slate-900 border-2 border-red-600 rounded-xl shadow-2xl w-full max-w-4xl overflow-hidden flex flex-col max-h-[90vh]" onClick={e => e.stopPropagation()}>
        ...
        <div className="flex border-b border-slate-700 shrink-0 overflow-x-auto">
            {['system', 'announce', 'manage', 'cleanup', 'users', 'feedback'].map(tab => (
                <button key={tab} onClick={()=>setActiveTab(tab)} className={`flex-1 py-3 px-4 font-bold whitespace-nowrap transition-colors uppercase text-sm ${activeTab===tab ? 'bg-red-600 text-white' : 'text-slate-400 hover:bg-slate-800'}`}>
                    {tab === 'system' && '⚙️ ระบบ'}
                    {tab === 'announce' && '📢 ประกาศ'}
                    {tab === 'manage' && '🔨 จัดการคน'}
        ...
        <div className="p-6 space-y-6 overflow-y-auto bg-slate-900/50">
            ...
            {/* ✅ Tab 1: System Config (ส่วนควบคุมใหม่) */}
            {activeTab === 'system' && (
                <div className="space-y-6">
                    <div className="p-5 bg-slate-800 rounded-xl border border-slate-700">
                        <h3 className="text-lg font-bold text-white mb-4 flex items-center gap-2">💰 ควบคุมระบบเติมเงิน (Top Up)</h3>
                        
                        {isConfigLoading ? <p className="text-slate-400">กำลังโหลดสถานะ...</p> : (
                            <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
                                {/* ปุ่มเปิด */}
                                <button 
                                    onClick={() => updateTopupStatus('open')}
                                    className={`p-4 rounded-xl border-2 transition-all flex flex-col items-center gap-2 ${topupStatus === 'open' ? 'bg-emerald-900/50 border-emerald-500 text-emerald-400 scale-105 shadow-lg shadow-emerald-500/20' : 'bg-slate-800 border-slate-600 text-slate-500 hover:bg-slate-700'}`}
                                >
                                    <span className="text-2xl">🟢</span>
                                    <span className="font-bold">เปิดใช้งานปกติ</span>
                                </button>

                                {/* ปุ่มปิดปรับปรุง */}
                                <button 
                                    onClick={() => updateTopupStatus('maintenance')}
                                    className={`p-4 rounded-xl border-2 transition-all flex flex-col items-center gap-2 ${topupStatus === 'maintenance' ? 'bg-amber-900/50 border-amber-500 text-amber-400 scale-105 shadow-lg shadow-amber-500/20' : 'bg-slate-800 border-slate-600 text-slate-500 hover:bg-slate-700'}`}
                                >
                                    <span className="text-2xl">🚧</span>
                                    <span className="font-bold">ปิดปรับปรุง</span>
                                    <span className="text-xs">(แจ้งขออภัยฯ)</span>
                                </button>

                                {/* ปุ่มปิดชั่วคราว */}
                                <button 
                                    onClick={() => updateTopupStatus('closed')}
                                    className={`p-4 rounded-xl border-2 transition-all flex flex-col items-center gap-2 ${topupStatus === 'closed' ? 'bg-red-900/50 border-red-500 text-red-400 scale-105 shadow-lg shadow-red-500/20' : 'bg-slate-800 border-slate-600 text-slate-500 hover:bg-slate-700'}`}
                                >
                                    <span className="text-2xl">⛔</span>
                                    <span className="font-bold">ปิดชั่วคราว</span>
                                    <span className="text-xs">(แจ้งปิดชั่วคราว)</span>
                                </button>
                            </div>
                        )}
                        <div className="mt-6 pt-4 border-t border-slate-700 text-center">
                            <p className="text-xs text-slate-400">สถานะปัจจุบันในระบบ:</p> 
                            <p className="text-xl font-black text-white uppercase mt-1">{topupStatus}</p>
                        </div>
                    </div>
                </div>
            )}
            ...
            {activeTab === 'announce' && (
                <div className="space-y-4">
                    <div><label className="text-xs font-bold text-slate-400 uppercase mb-1 block">หัวข้อประกาศ</label><input value={title} onChange={e=>setTitle(e.target.value)} className="w-full bg-slate-800 border border-slate-600 rounded-lg p-3 text-white focus:border-red-500 outline-none" placeholder="เช่น แจ้งปิดปรับปรุง..." /></div>
