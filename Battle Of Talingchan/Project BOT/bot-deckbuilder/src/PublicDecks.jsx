...
// --- Imported Components (จากไฟล์กลาง) ---
import SettingsDrawer from './components/SettingsDrawer';
import ProfileSetupModal from './components/ProfileSetupModal';
import RatingBadge from './components/RatingBadge';
import DeckListModal from './components/DeckListModal'; // 🟢 Import แล้ว
import { 
    MenuIcon, ShoppingBagIcon, UserCogIcon, CloseIcon, 
    SunIcon, MoonIcon, MessageIcon, StoreIcon, HomeIcon, 
    UsersIcon, DeckIcon, ClearIcon, HeartIcon, CameraIcon, 
...
// === Local UI Components (Style เฉพาะหน้านี้) ===
...
// === Feedback Modal (เก็บไว้ในไฟล์นี้เพราะยังไม่ได้ทำเป็น Global Shared) ===
    ...
    <div className="fixed inset-0 bg-black/90 backdrop-blur-sm flex items-center justify-center z-[200] p-0 md:p-4">
      <div className="bg-white dark:bg-slate-900 border-0 md:border border-slate-300 dark:border-emerald-500/30 rounded-none md:rounded-xl shadow-2xl w-full h-full flex flex-col max-w-[95vw] max-h-[100vh] md:max-h-[90vh] overflow-hidden">
        ...
        <div className="flex border-b border-slate-200 dark:border-slate-700 shrink-0">
          <button onClick={() => setActiveTab("deck")} className={`flex-1 py-3 text-sm md:text-base font-bold transition-colors relative ${activeTab === "deck" ? "text-emerald-600 dark:text-emerald-400 bg-emerald-50 dark:bg-emerald-900/10" : "text-slate-500 dark:text-slate-400 hover:bg-slate-50 dark:hover:bg-slate-800"}`}>🔍 ดูการ์ด & สถิติ {activeTab === "deck" && <div className="absolute bottom-0 left-0 w-full h-0.5 bg-emerald-500"></div>}</button>
          <button onClick={() => setActiveTab("comments")} className={`flex-1 py-3 text-sm md:text-base font-bold transition-colors relative ${activeTab === "comments" ? "text-blue-600 dark:text-blue-400 bg-blue-50 dark:bg-blue-900/10" : "text-slate-500 dark:text-slate-400 hover:bg-slate-50 dark:hover:bg-slate-800"}`}>💬 ความคิดเห็น {activeTab === "comments" && <div className="absolute bottom-0 left-0 w-full h-0.5 bg-blue-500"></div>}</button>
        </div>
  ...
  return (
    <div className="h-screen flex flex-col text-slate-900 dark:text-gray-200 bg-slate-100 dark:bg-black">
      ...
      <SettingsDrawer
        ...
        onOpenMyDecks={() => setIsDeckListModalOpen(true)} // 🟢 เพิ่ม: เปิด Modal My Decks
      />
      ...
      {/* 🟢 [ใหม่] เพิ่ม DeckListModal ตรงนี้ */}
      <DeckListModal
