...
export default function PublicDecks() {
  ...
  useEffect(() => {
    if (userProfile?.email) {
        ...
        // 2. ดึง Stats (จาก Supabase) - ทำเหมือน App.jsx
        const fetchStats = async () => {
             // ต้องมี supabase import เข้ามาก่อนนะ (เดี๋ยวผมเพิ่มให้ด้านบน)
             // เนื่องจากไฟล์นี้ใช้ firebase เป็นหลัก การดึง stats จาก supabase อาจจะต้องทำแยก
             // หรือถ้าคุณไม่ได้ใช้ supabase ในไฟล์นี้ ก็อาจจะละไว้ก่อน
             // แต่ถ้าอยากให้ยศขึ้น ก็ต้อง import supabase มาดึงข้อมูลครับ
        };
        // fetchStats(); // ถ้าต้องการ
    }
  }, [userProfile]);
