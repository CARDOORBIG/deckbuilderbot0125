...
import { supabase } from '../supabaseClient'; // ถอยกลับ 1 folder
import { doc, getDoc } from 'firebase/firestore';
import { db } from '../firebase'; // ถอยกลับ 1 folder
import { getCardImageUrl } from '../utils/auctionUtils';
import TimeLeft from './TimeLeft';
import RatingBadge from './RatingBadge';
import { 
    ChevronLeftIcon, ChevronRightIcon, ExpandIcon, 
    ChatBubbleIcon, CloseIcon, ShoppingBagIcon, GavelIcon, SendIcon 
} from './Icons';
                ...
                <div className="w-full md:w-1/3 h-[50vh] md:h-full flex flex-col border-l border-slate-200 dark:border-slate-800 bg-white dark:bg-slate-900 min-h-0">
                    ...
                    <div className="p-3 bg-white dark:bg-slate-900 border-t border-slate-200 dark:border-emerald-500/20">
                         <div className="flex items-center justify-between mb-3">
                            ...
                            {/* Buttons: Show if not owner and not ended */}
                            {userProfile?.email !== auction.seller_email && !isEnded && (
                                <div className="flex gap-2">
                                    {auction.buy_now_price > 0 && (
                                        <button onClick={() => onBuyNow(auction)} className="px-3 py-1.5 bg-pink-100 text-pink-700 rounded-lg text-xs font-bold shadow-sm hover:bg-pink-200 transition-colors">
                                            Buy ฿{auction.buy_now_price.toLocaleString()}
                                        </button>
                                    )}
                                    {(auction.min_bid_increment > 0) && (
                                        <button onClick={() => onBid(auction)} className="px-4 py-1.5 btn-fire text-white rounded-lg text-xs font-bold shadow-lg transition-transform active:scale-95">
