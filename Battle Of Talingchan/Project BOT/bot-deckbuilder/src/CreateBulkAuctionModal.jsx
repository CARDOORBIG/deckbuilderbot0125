...
// --- Icons ---
const BigCheckIcon = () => (
  <svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 24 24" fill="currentColor" className="w-20 h-20 text-emerald-500">
...
export default function CreateBulkAuctionModal({ isOpen, onClose, userProfile }) {
  ...
  // การตั้งค่าประมูล (เหมือน Single)
  ...
  const handleValidateAndCheck = () => {
    ...
    if (!price || parseInt(price) < 0) return alert("กรุณาระบุราคาเริ่มต้นที่ถูกต้อง");
    ...
    if (buyNow && parseInt(buyNow) <= parseInt(price)) return alert("ราคา Buy Now ต้องสูงกว่าราคาเริ่มต้นครับ");
  ...
  const submitBulkAuction = async () => {
    ...
    try {
        ...
        const uploadPromises = imageFiles.map(async (file) => {
            ...
            const fileName = `bulk_${Date.now()}_${Math.random().toString(36).substring(2)}.${fileExt}`;
        ...
        const payload = {
          ...
          card_id: `BULK-${Date.now()}`, // สร้าง ID ปลอมสำหรับ Bulk
          card_name: title.trim(),
          card_image_path: 'CUSTOM_ITEM', // Flag บอกว่าเป็น Custom
          ...
          proof_image: JSON.stringify(uploadedUrls), // เก็บรูปทั้งหมดในนี้
        ...
        {isSuccess ? (
             <div className="flex flex-col items-center justify-center h-96 text-center animate-fade-in-up">
                ...
                <p className="text-slate-500 dark:text-slate-400 text-lg">
                    สินค้าของคุณเข้าสู่ลานประมูลแล้ว 📦
                </p>
            </div>
        ) : (
            // หน้าฟอร์ม
            <>
                <h2 className="text-xl font-bold mb-4 text-slate-900 dark:text-white flex items-center gap-2">
                   <ShoppingBagIcon /> เปิดประมูล (ยกกล่อง / อื่นๆ)
                </h2>
                ...
                <div className="bg-amber-50 dark:bg-amber-900/20 border border-amber-300 dark:border-amber-800 p-4 rounded-xl mb-5 text-sm text-amber-800 dark:text-amber-200">
                    ...
                    <ul className="list-disc list-inside space-y-1">
                        <li>สำหรับสินค้าที่ <b>ไม่มีในฐานข้อมูล</b> (เช่น ยกกล่อง, Playmat)</li>
                        <li><b>จำเป็นต้อง</b> ถ่ายรูปสินค้าจริงให้ชัดเจน</li>
                ...
                <div className="space-y-4">
                    ...
                    <div className="grid grid-cols-2 gap-4">
                        <div><label className="block text-sm text-slate-500 dark:text-gray-400 mb-1">ราคาเริ่มต้น (บาท)</label><input type="number" value={price} onChange={e => setPrice(e.target.value)} className="w-full p-2 rounded border bg-white dark:bg-slate-800 dark:border-slate-700 dark:text-white outline-none focus:border-emerald-500 font-mono text-lg font-bold text-emerald-600" min="0" /></div>
                        <div><label className="block text-sm text-slate-500 dark:text-gray-400 mb-1">ระยะเวลา (ชม.)</label><input type="number" value={hours} onChange={e => setHours(e.target.value)} className="w-full p-2 rounded border bg-white dark:bg-slate-800 dark:border-slate-700 dark:text-white outline-none focus:border-amber-500 font-mono" min="1" /></div>
                    </div>
        ...
        <div className="fixed inset-0 z-[1200] flex items-center justify-center p-4 bg-black/60 backdrop-blur-sm animate-fade-in" onClick={(e) => e.stopPropagation()}>
            <div className="bg-white dark:bg-slate-900 border-[3px] border-blue-500 rounded-2xl p-6 max-w-sm w-full shadow-[0_0_30px_rgba(59,130,246,0.4)] transform scale-100 animate-bounce-in relative overflow-hidden flex flex-col items-center text-center">
                ...
                <div className="flex gap-3 w-full">
                    ...
                    <button onClick={submitBulkAuction} disabled={loading} className="flex-1 py-3 bg-blue-600 text-white font-bold rounded-xl shadow-lg hover:bg-blue-500 transition-transform active:scale-95">{loading ? "กำลังบันทึก..." : "ยอมรับและลงขาย"}</button>
                </div>
            </div>
        </div>
      )}
            ...
            <div className="bg-white dark:bg-slate-900 border-[3px] border-amber-500 rounded-2xl p-6 max-w-sm w-full shadow-[0_0_30px_rgba(245,158,11,0.4)] transform scale-100 animate-bounce-in relative overflow-hidden flex flex-col items-center text-center">
                ...
                <div className="text-slate-600 dark:text-slate-300 mb-6 space-y-3 text-sm text-left bg-amber-50 dark:bg-amber-900/20 p-4 rounded-lg border border-amber-200 dark:border-amber-800">
                    ...
                    <ul className="list-disc list-inside space-y-1 text-xs">
                        ...
                        <li>หากตรวจพบการทุจริต ระบบจะระงับบัญชีถาวร</li>
                    </ul>
                </div>
                <div className="flex gap-3 w-full">
                    ...
                    <button onClick={submitBulkAuction} disabled={loading} className="flex-1 py-3 bg-amber-500 text-white font-bold rounded-xl shadow-lg hover:bg-amber-600 transition-transform active:scale-95">{loading ? "กำลังบันทึก..." : "ยืนยันลงขาย"}</button>
                </div>
            </div>
        </div>
      )}
